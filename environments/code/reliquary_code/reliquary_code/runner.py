...
def _program(source: str, cpu_seconds: int, memory_bytes: int) -> str:
    """The child's whole program: set its own limits, then run the submission.

    The limits used to be applied by a `preexec_fn`, which runs in the forked
    child between fork and exec. That is unsafe from a threaded caller, and not
    theoretically: a fork carries only the calling thread, so a lock another
    thread held at fork time is inherited locked with no owner to release it,
    and the child blocks on it forever. The child then never reaches exec, never
    writes the error pipe `Popen.__init__` is reading, and the parent blocks in
    the constructor — before `communicate`'s timeout exists to save it. There is
    no timeout on that path. Measured here: a twelve-thread probe deadlocked
    after roughly an hour of grading and stayed stuck, GPU idle, until killed.

    Setting the limits inside the child instead removes the fork-time callback
    entirely. Two consequences, both conservative and both deliberate:
    `RLIMIT_CPU` is cumulative from process start, so the interpreter's own
    startup now counts against the submission's budget, and `RLIMIT_AS` is
    applied once the interpreter has already allocated, so what remains is
    slightly less than the nominal cap. A submission is judged a little more
    harshly than before, never more leniently.

    `source` here is the worker script, not the candidate's code — the
    submission travels in stdin and the worker execs it. It is still embedded
    and `exec`'d rather than concatenated, because prepending to a source text
    breaks any unit that opens with a future statement, and the worker script
    is not a file this function should have to read before wrapping.
    """
    return (
        "import resource\n"
        ...
        f"exec(compile({source!r}, '<submission>', 'exec'), "
        "{'__name__': '__main__'})\n"
    )


def _limits(cpu_seconds: int, memory_bytes: int) -> Callable[[], None]:
    """Retained for the tests that assert what the limits are, unused by
    `_spawn`. The values live here; `_program` is what applies them."""
...
def _spawn(
    ...
    *,
    ...
    No `preexec_fn`: the limits travel inside the child's own program, see
    `_program`. The earlier version applied them from a fork-time callback on
    the assumption that the caller is single-threaded batch or test code. That
    assumption does not hold for a validator, which grades concurrently, and
    it deadlocks when it is wrong — without a timeout, because the parent is
    still inside `Popen.__init__` when it happens.
    ...
    try:
        proc = subprocess.Popen(
            [
                ...
                "-S",
                "-c",
                _program(source, cpu_seconds, memory_bytes),
            ],
