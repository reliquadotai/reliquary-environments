...
_ALLOWED_IMPORT_ROOTS = {
    # `__future__` carries no capability — it holds `_Feature` records and
    # version tuples, nothing callable on the host. It is here because a future
    # statement is compiled, not merely imported, and leaving it out scored a
    # correct solution zero for opening with `from __future__ import
    # annotations`: an idiom, not a behaviour. A false negative teaches the
    # policy that a standard line is wrong, which is worse than the import
