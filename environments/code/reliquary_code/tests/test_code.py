...
def test_grading_from_many_threads_does_not_deadlock() -> None:
    """The defect this package was carrying, in the shape that revealed it.

    Limits used to be set by a `preexec_fn`, which runs between fork and exec.
    A fork carries only the calling thread, so a lock another thread held at
    fork time arrives locked with no owner: the child blocks forever, never
    execs, never writes the error pipe the parent is reading, and the parent
    hangs inside `Popen.__init__` — before `communicate`'s timeout exists.
    A twelve-thread probe hit it after about an hour of grading and stayed
    stuck. A validator grades concurrently, so the single-threaded assumption
    the runner used to make was not one it was entitled to.

    The bound is generous on purpose: this test fails by hanging, and what it
    is worth is the difference between "slow" and "never".
    """
    from concurrent.futures import ThreadPoolExecutor

    started = time.monotonic()
    with ThreadPoolExecutor(max_workers=12) as pool:
        results = list(pool.map(lambda _: run_cases(INCREMENT, CASES), range(48)))
    assert results == [[True, True]] * 48
    assert time.monotonic() - started < 120


def test_a_future_import_is_not_a_forbidden_import() -> None:
    """`__future__` was missing from the sandbox's allowlist, so a solution
    opening with `from __future__ import annotations` scored zero however
    correct it was — graded `forbidden_import` for an idiom, not a behaviour.
    The module holds `_Feature` records and version tuples and nothing
    callable, so allowing it costs the sandbox nothing."""
    source = "from __future__ import annotations\n\ndef increment(n: int) -> int:\n    return n + 1\n"
    assert run_cases(source, CASES) == [True, True]
...
def test_artifact_manifest_hashes_installed_files() -> None:
    """Verified in place, against the files as installed.

    Regenerating through a repository-root helper would only prove the helper
    is self-consistent; a consumer of the wheel has the manifest and the files
    and nothing else, so the check has to be the one they can run.
    """
    artifact = json.loads((PACKAGE_ROOT / "artifact.json").read_text())
    assert artifact["entrypoints"]["taskset"] == "reliquary_code:CodeTaskset"
    source_manifest = PACKAGE_ROOT.parent / "environment.toml"
    assert (
...
def test_the_declared_budget_is_the_measured_one() -> None:
    """8,192 rather than the maths sibling's 24,576, and the direction is the
    finding: more budget measured *worse* here (band 45.8% against 42.7%),
    because the groups that stop being truncated become unanimous rather than
    harder. The example has to grant exactly what is declared, and the run has
    to have room for it plus a prompt."""
    import tomllib

    policy = tomllib.loads(
        (PACKAGE_ROOT.parent / "environment.toml").read_text(encoding="utf-8")
    )["policy"]
    assert policy["max_new_tokens"] == 8192
    assert policy["max_new_tokens_rationale"].strip()

    run = tomllib.loads(
        (PACKAGE_ROOT.parent / "examples/prime_rl/rl.toml").read_text()
    )
    for phase in ("train", "eval"):
        sampling = run["orchestrator"][phase]["sampling"]
        assert sampling["max_completion_tokens"] == policy["max_new_tokens"]
...
def test_repository_compatibility_declares_the_pins_this_package_uses() -> None:
    """The release entry lands with the release, so it is not asserted here.

    What can be asserted before one exists is that the package asks for the
    exact Verifiers the repository reviewed: a range would let a wheel built
    tomorrow grade differently from the one that was qualified.
    """
    import tomllib
    ...
    assert "verifiers==0.3.1" in project["project"]["dependencies"]
    assert (
