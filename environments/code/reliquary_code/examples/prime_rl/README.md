...
gap in this reference run. Set `RELIQUARY_ENVS_PATH` to a checkout of this
repository (`reliquary-environments`) before running the block below.
...
uv pip install --python prime-rl/.venv/bin/python --no-deps \
  -e "$RELIQUARY_ENVS_PATH/environments/code/reliquary_code"
```

`reliquary-code` has never had a tagged release, so there is no `v0.1.0a2`
GitHub release to fetch a wheel from — installing from a release-wheel URL
is not possible yet. The command above installs the package directly from
its directory in this repository instead (the same source `uv sync
--locked` in the package's own README installs). Once `v0.1.0a2` is cut,
switch the last line to fetch that release's wheel
(`reliquary_code-0.1.0a2-py3-none-any.whl`) the way
