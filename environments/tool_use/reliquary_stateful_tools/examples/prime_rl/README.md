...
This is the first pinned Prime-RL training lane for `reliquary-stateful-tools`.
It uses Prime-RL's native Verifiers environment boundary, Qwen3 renderer,
GRPO, zero-signal group rejection, held-out online evaluation, and native
...
be installed. Docker enforces the taskset's deny-all network policy; Verifiers
correctly refuses to run this policy with its unsandboxed subprocess runtime.
...
```bash
PRIME_CONFIG_PATH="$RELIQUARY_ENVS_PATH/environments/tool_use/reliquary_stateful_tools/examples/prime_rl/rl.toml"

prime-rl/.venv/bin/python -c "import verifiers.v1 as vf; c=vf.taskset_config_type('reliquary-stateful-tools'); print(next(iter(vf.load_taskset(c(id='reliquary-stateful-tools', split='train')))).key)"

prime-rl/.venv/bin/rl @ "$PRIME_CONFIG_PATH" \
  --model.name "$MODEL_SNAPSHOT_PATH" --dry-run True \
  --output-dir outputs --run.name reliquary-stateful-tools-dry-run

prime-rl/.venv/bin/rl @ "$PRIME_CONFIG_PATH" \
  --model.name "$MODEL_SNAPSHOT_PATH" --max-steps 5 \
  --output-dir outputs --run.name reliquary-stateful-tools-smoke

prime-rl/.venv/bin/rl @ "$PRIME_CONFIG_PATH" \
  --model.name "$MODEL_SNAPSHOT_PATH" \
  --output-dir outputs --run.name reliquary-stateful-tools-qwen3-4b-run-1
```

The five-step smoke must produce valid tool calls, non-zero mixed-outcome
groups, a checkpoint, and clean train/eval traces before the 100-step run.
Keep Prime-RL's resolved configs, metrics, traces, and checkpoint manifests as
the evidence bundle; do not invent a parallel logging format.
...
signal, bounded off-policy lag, evaluation before training and every 15 steps,
and periodic checkpoints. The 32-prompt LoRA run is deliberately sized for a
small proof, not copied from INTELLECT-3's 512-H200 GLM run.

Prime-RL v0.9.0 uses its current DPPO+KL trainer loss with GRPO advantages. It
is therefore accurate to report **Prime-RL v0.9.0 GRPO**, not an exact
...
justify them. This procedural taskset is effectively infinite, while Prime's
difficulty-pool sampler requires a finite taskset.
...
evaluate `qualification` once. Report Avg@16 and per-family means; pass@16 alone can
overstate the gain. Compare the base snapshot and selected checkpoint with the
same renderer, sampling values, limits, and task indices.
