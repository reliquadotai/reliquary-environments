"""The agent's toolkit: what a support agent can do to the carrier's records.

Vendored from τ²-bench `src/tau2/domains/telecom/tools.py`, except for the
changes below.

- The imports point at this package's `toolkit`, and `get_today` comes from
  there rather than from `tau2.domains.telecom.utils`.
- `loguru` is gone. Four calls logged a suspension, a resumption and two
  roaming toggles; none of them was read by anything.
- The `__main__` block is gone.
- `_apply_one_time_charge` no longer names its draft bill after a random UUID.
  See `_draft_bill_id`.

The `assert_*` methods at the foot of the file, and `suspend_line_for_overdue_
bill` above them, are deliberately undecorated: an undecorated method is not a
tool, so the agent can neither read its own mark nor set the scenario it is
...
from reliquary_telecom_solo._tau2.data_model import (
    Bill,
    BillStatus,
    Customer,
    Device,
...
from reliquary_telecom_solo._tau2.toolkit import (
    ToolKitBase,
    ToolType,
    get_today,
    is_tool,
...
class TelecomTools(ToolKitBase):
    ...
    @staticmethod
    def _draft_bill_id(customer_id: str, period_start: date) -> str:
        """Name a draft bill after the bill, not after chance.

        Upstream built this id from `uuid.uuid4()`. No assertion reads a bill
        id, so the reward never noticed — but the id is returned to the agent
        and therefore lands in the token transcript, and a network that pays
        for a rollout by replaying its transcript cannot replay one whose
        tokens differ every time it runs. A customer has at most one draft bill
        per billing period, so the pair identifies it exactly.
        """
        digest = sha256(f"{customer_id}:{period_start.isoformat()}".encode())
        return f"B{digest.hexdigest()[:8]}"
    ...
    def _apply_one_time_charge(
        ...
        if not draft_bill:
            ...
            new_bill_id = self._draft_bill_id(customer_id, next_month)
            draft_bill = Bill(
                bill_id=new_bill_id,
