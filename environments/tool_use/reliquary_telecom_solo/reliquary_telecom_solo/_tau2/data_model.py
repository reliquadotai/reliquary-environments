"""The carrier's side of the world: plans, customers, lines, bills, devices.

Vendored verbatim from τ²-bench `src/tau2/domains/telecom/data_model.py`,
except for the changes below.

- The imports point at this package's `toolkit` instead of `tau2.environment`
  and `tau2.utils`.
- `get_db` and the `__main__` block are gone; the database is loaded by
  `reliquary_telecom_solo.corpus` from the TOML shipped in the wheel, not from
  a path resolved against an environment variable at import time.
"""
