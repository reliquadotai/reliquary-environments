"""The user's toolkit: what can be done to the phone in the user's hand.

Vendored verbatim from τ²-bench `src/tau2/domains/telecom/user_tools.py`, with
only the import block re-pointed at this package.

In dual-control τ² these belong to the user simulator and the agent can only
ask for them in prose. In solo mode they are the agent's, which is what makes
the episode a single actor working a ticket — and what makes it replayable,
since the other actor was a language model.

Nothing here samples. `_run_speed_test` reads like a simulation with a random
component and is not one: the speed is the midpoint of a fixed range for the
connected technology, scaled by fixed factors for signal, VPN and data saver.
`assert_internet_speed`, which 254 tasks score on, therefore returns the same
verdict in every process.
"""

from typing import Any, Dict, Literal, Optional, Tuple, Union

from reliquary_telecom_solo._tau2.user_data_model import (
    APNNames,
    APNSettings,
    AppPermissions,
    AppStatus,
...
from reliquary_telecom_solo._tau2.toolkit import ToolKitBase, ToolType, is_tool
